# Student Digital Ambassador Project
**Course:** Digital Literacy & Professional Presence  
**Name:** [ANIRUDH VAMSI G]  
**Branch/Year:** [M.TECH-BIOINFORMATICS], [2025-2030]

## Project Overview
This repository contains the completed tasks for the Digital Literacy course. As a Student Digital Ambassador, I have developed resources to promote digital awareness, established a professional online presence, and explored tools for academic collaboration and cybersecurity.

## Repository Structure
* `task-1-presentation/`: Digital Literacy Infographic (PNG/PDF).
* `task-2-portfolio/`: Screenshots of GitHub, LinkedIn, and [Third Platform].
* `task-3-platforms/`: Coding challenge results, Google Form screenshots, and Response Sheet.
* `task-4-email-etiquette/`: Professional email drafts and Social Media Checklist.
* `task-5-cybercrime/`: Cybercrime Case Study and Prevention Guide.

## Digital Literacy Quiz
As part of Task 3, I have developed a quiz for batchmates:
* **Quiz Link:** [https://forms.gle/bcVFro7LZL6nQoVc7]

---
*Generated as part of the Digital Literacy Module Requirements.*
